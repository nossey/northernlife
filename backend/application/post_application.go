...
func (app *PostApplication) GetPostList(page int, getType domain.GetPostType) (result PostGetResult) {
	perPageCount := 10
	result.PerPageCount = perPageCount
	offset := perPageCount * (page - 1)
	result.Posts = dataaccessor.PostAccessor.GetPostList(offset, perPageCount, domain.Published)
